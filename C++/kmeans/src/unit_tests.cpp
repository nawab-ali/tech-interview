...
TEST(KMeansTest, ClusterCalculateCentroid) {
    ...
    vector<double> centroid_coordinates =
        cluster.get_centroid().get_coordinates();

    for (int i = 0; i < point4.get_coordinates().size(); i++) {
        ASSERT_NEAR(point4.get_coordinates()[i], centroid_coordinates[i],
                    0.00001);
    }
}
...
TEST(KMeansTest, KMeansInitClusterCentroidClustersSizeMoreThanPointsSize) {
    ...
    EXPECT_THROW(init_cluster_centroid<double>(clusters, points),
                 invalid_argument);
}
...
TEST(KMeansTest, KMeansGetNearestClusterEmptyClusters) {
    ...
    EXPECT_THROW(get_nearest_cluster<double>(point, clusters),
                 invalid_argument);
}
...
TEST(KMeansTest,
     KMeansCategorizePointsIntoClustersNearestClusterIdMoreThanClustersSize) {
    vector<double> v1{3.5, 1.1, 5.2};
    vector<double> v2{4.5, 1.2, 6.2};
    vector<double> v3{5.5, 1.3, 7.2};
    vector<double> v4{6.5, 1.4, 8.2};
    ...
    vector<Point<double>> points{point1, point2, point3, point4,
    ...
    EXPECT_THROW(categorize_points_into_clusters<double>(points, clusters),
                 out_of_range);
}

TEST(KMeansTest,
     KMeansCategorizePointsIntoClustersNearestClusterIdLessThanZero) {
    vector<double> v1{3.5, 1.1, 5.2};
    vector<double> v2{4.5, 1.2, 6.2};
    vector<double> v3{5.5, 1.3, 7.2};
    vector<double> v4{6.5, 1.4, 8.2};
