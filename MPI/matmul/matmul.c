...
void matmul(float* recvbuf_a, float* b, float* sendbuf_c, const int recvcount, const int M, const int K, const int N);
...
int main(int argc, char** argv) {
	...
	MPI_Scatter(a, sendcount, MPI_FLOAT, recvbuf_a, recvcount, MPI_FLOAT, 0, MPI_COMM_WORLD);

	matmul(recvbuf_a, b, sendbuf_c, recvcount, M, K, N);

	sendcount = recvcount = (M*N)/numtasks;
	MPI_Gather(sendbuf_c, sendcount, MPI_FLOAT, c, recvcount, MPI_FLOAT, 0, MPI_COMM_WORLD);
	if (rank == 0) {
		print_matrix(c, M, N);
	}
