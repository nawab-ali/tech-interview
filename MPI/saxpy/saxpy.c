...
void allocate_buffer_memory(float **recvbuf_x, float **recvbuf_y,
                            const int numtasks, const int bufsize);
...
int main(int argc, char **argv) {
    ...
    MPI_Scatter(x, sendcount, MPI_FLOAT, recvbuf_x, recvcount, MPI_FLOAT, 0,
                MPI_COMM_WORLD);
    MPI_Scatter(y, sendcount, MPI_FLOAT, recvbuf_y, recvcount, MPI_FLOAT, 0,
                MPI_COMM_WORLD);

    saxpy(a, recvbuf_x, recvbuf_y, recvcount);

    MPI_Gather(recvbuf_y, sendcount, MPI_FLOAT, y, recvcount, MPI_FLOAT, 0,
               MPI_COMM_WORLD);
    if (rank == 0) {
        printf("\n");
